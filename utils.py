...
from core import Normalizer, RobustL1Loss, RobustL2Loss, sampled_softmax
from segments import ResidualNetwork
...
def results_regression(
    ...
    for j in range(ensemble_folds):
        ...
        with torch.no_grad():
            #Orig
            # idx, comp, hs, y_test, output = model.predict(generator=test_generator,)
            #MPHE
            idx, comp, hs, origs, y_test, output = model.predict(generator=test_generator,)
    ...
    #MPHE
    core = {"id": idx, "composition": comp, "Initial loading": [h[0] for h in hs],\
            "Final loading": [h[1] for h in hs],"target": y_test,\
            "origin":[orig.item() for orig in origs]}
