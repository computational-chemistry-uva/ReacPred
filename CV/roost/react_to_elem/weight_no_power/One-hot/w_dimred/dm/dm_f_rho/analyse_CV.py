...
for key in dfs.keys():
    ...
    df = dfs[key]
    # print(df.info())
    plt.scatter(df['target'], df['pred_0'], color = 'g', s = 1)
    
plt.xlim([0,6])
plt.ylim([0,6])
x = [0,10]
plt.plot(x,x, color = 'k', linestyle = '--', linewidth = 1)
plt.xlabel(r'$\rho_{target},g/cm^{3}$', fontsize = 20)
plt.ylabel(r'$\rho_{prediction}, g/cm^{3}$', fontsize = 20)
