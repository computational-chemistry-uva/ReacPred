...
class BaseModelClass(nn.Module, ABC):
    ...
    def evaluate(
        ...
        with trange(len(generator), disable=(not verbose)) as t:
            ...
            #MPHE
            # we do not need batch_comp, batch_hydration_states_res, or batch_ids when training
            for input_, target, _, _, _, _ in generator:
    ...
    def predict(self, generator, verbose=False):
        ...
        with torch.no_grad():
            with trange(len(generator), disable=(not verbose)) as t:
                #Orig
                # for input_, target, batch_comp, batch_hs, batch_ids in generator:
                #MPHE
                for input_, target, batch_comp, batch_hs, batch_orig, batch_ids in generator:
